...
def test_enqueue_list_assign_and_cancel(client: TestClient):
    ...
    res = client.post('/api/v1/robotics/schedule/enqueue', json=payload)
    ...
    res = client.get('/api/v1/robotics/schedule/queue')
    assert res.status_code == 200
    q = res.json()
    tasks = q['data']['tasks']
    ...
    res = client.post('/api/v1/robotics/schedule/assign')
    assert res.status_code == 200
    assigned = res.json()
    ...
    # Cancel pending task
    res = client.delete('/api/v1/robotics/schedule/cancel/task-66-2')
    assert res.status_code == 200
    cancelled = res.json()
